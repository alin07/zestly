...
const EditListingPage: React.FC = () => {
  ...
  const { data: listingDetailData, loading: listingLoading } = useQuery(GET_LISTING, {
    variables: { id: id! },
    skip: !id,
  });
  ...
  // Initialize form data when listing data is loaded
  useEffect(() => {
    if (listingDetailData?.listing && !initialized) {
      const listing = listingDetailData.listing;
      
      // Check if user owns this listing
      if (listing.agent.id !== user?.id) {
      ...
      if (listing.property.address) {
        setAddressData({
          ...
          unit: listing.property.address.unit || '',
          city: listing.property.address.city || '',
          state: listing.property.address.state || '',
          zipCode: listing.property.address.zipCode || '',
          country: listing.property.address.country || 'United States',
      ...
      setPropertyData({
        ...
        sqft: listing.property.sqft?.toString() || '',
        lotSizeSqft: listing.property.lotSizeSqft?.toString() || '',
        yearBuilt: listing.property.yearBuilt?.toString() || '',
        parkingSpaces: listing.property.parkingSpaces?.toString() || '',
        features: listing.property.features || [],
        description: listing.property.description || '',
        hoaFee: listing.property.hoaFee?.toString() || '',
        propertyTaxAnnual: listing.property.propertyTaxAnnual?.toString() || '',
        zoning: listing.property.zoning || '',
        mlsNumber: listing.property.mlsNumber || '',
      });
  ...
  }, [listingDetailData, user, initialized]);
